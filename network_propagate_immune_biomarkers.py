...
exec(open('/Users/yashwipassary/Downloads/ML_Bio_Project//utilities/network_utilities_ver4.py').read())
exec(open('/Users/yashwipassary/Downloads/ML_Bio_Project//utilities/useful_utilities.py').read())
start = time.ctime()


## Initialize
string_cutoff = 700
fo_dir = '/Users/yashwipassary/Downloads/ML_Bio_Project/result'

## Prepare data

#  Load immunotherapy biomarkers
ib = pd.read_csv('/Users/yashwipassary/Downloads/ML_Bio_Project/data/Marker_summary.txt', sep='\t')


#  Construct Network (STRING network v11)
print('constructing STRING PPI network, %s'%time.ctime())
tmp_G = nx.Graph()
annotation = pd.read_csv('/Users/yashwipassary/Downloads/ML_Bio_Project/data/9606.protein.aliases.v11.0.txt', sep="\t", header=None, comment="#", names=["string_protein_id", "alias", "source"], usecols=[0, 1, 2]) # STRING network ensembl-geneID mapping
net = pd.read_csv('/Users/yashwipassary/Downloads/ML_Bio_Project/data/9606.protein.links.v11.0.txt', sep=' ') # STRING network
nodes1 = net.values[:,0]
nodes2 = net.values[:,1]
scores = net.values[:,2]
